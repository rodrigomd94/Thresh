...
use std::str::FromStr;
use pallas_primitives::{conway::Value, AssetName, Hash, NonEmptyKeyValuePairs, PolicyId, PositiveCoin};
...
pub fn convert_to_multiasset_positive_coin(
    ...
    let btree_result: BTreeMap<Hash<28>, BTreeMap<AssetName, PositiveCoin>> = out_assets
        ...
        .fold(BTreeMap::new(), |mut acc, multiasset| {
            ...
            let asset_map: BTreeMap<AssetName, PositiveCoin> = multiasset
                ...
                .into_iter()
                .filter_map(|asset| {
                    let asset_name = AssetName::from(pallas_primitives::Bytes::from(asset.name.to_vec()));
                    let coin_value = asset.output_coin;
                    
                    // Skip zero values by returning None
                    PositiveCoin::try_from(coin_value).ok().map(|positive_coin| {
                        (asset_name, positive_coin)
                    })
                })
                .collect();
                ...
                for (name, coin) in &asset_map {
                    existing_map.entry(name.clone())
                        .and_modify(|existing| {
                            ...
                            *existing = PositiveCoin::try_from(new_value).expect("Combined value should be positive");
                        })
                        .or_insert(*coin);
                }
            })
    ...
    let policy_pairs: Vec<(Hash<28>, NonEmptyKeyValuePairs<AssetName, PositiveCoin>)> = btree_result
        .into_iter()
    ...
    NonEmptyKeyValuePairs::try_from(policy_pairs)
        .map_err(|_| ConversionError::EmptyAssets)
}
